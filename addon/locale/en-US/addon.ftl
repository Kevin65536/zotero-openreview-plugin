startup-begin = Zotero OpenReview loading...
startup-finish = Zotero OpenReview ready
menuitem-import-openreview = Import from OpenReview...
prefs-title = OpenReview
prefs-table-title = Title
prefs-table-detail = Detail

# Import Dialog
dialog-title = Import from OpenReview
dialog-url-label = Workshop/Venue URL:
dialog-collection-label = Collection Name:
dialog-filter-label = Paper Filter
dialog-filter-all = All Papers
dialog-filter-oral = Oral Only
dialog-filter-poster = Poster Only
dialog-options-label = Options
dialog-download-pdfs = Download PDFs
dialog-import-reviews = Import reviews and comments as notes
dialog-accepted-only = Only import accepted papers
dialog-skip-duplicates = Skip papers already in library
dialog-import-button = Import
dialog-cancel-button = Cancel

# Progress messages
progress-title = Importing from OpenReview
progress-parsing-url = Parsing URL...
progress-keep-visible = Import is in progress. This window will stay open until the current import finishes so the visual progress is not lost.
progress-fetching-papers = Fetching papers from OpenReview...
progress-no-papers = No papers found in this venue
progress-creating-collection = Creating collection...
progress-importing = Importing papers
progress-complete = Import complete! Imported: { $imported }, Skipped: { $skipped }, Failed: { $failed }
progress-error = Error

# Error messages
error-title = OpenReview Import Error
error-no-url = Please enter an OpenReview URL
error-no-collection-name = Please enter a collection name
error-invalid-url = Invalid OpenReview URL. Please enter a valid venue or workshop URL.